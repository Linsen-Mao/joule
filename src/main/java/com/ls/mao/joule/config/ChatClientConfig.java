...
public class ChatClientConfig {
    ...
    @Bean
    public ChatClient chatClient(Builder builder, Optional<VectorStore> vectorStore) {
        ...
        Builder configuredBuilder = builder.defaultAdvisors(new MessageChatMemoryAdvisor(new InMemoryChatMemory()));

        if (isPgVectorActivated && vectorStore.isPresent()) {
            configuredBuilder = configuredBuilder.defaultSystem(systemPrompt)
                    .defaultAdvisors(new QuestionAnswerAdvisor(vectorStore.get()));
        }
